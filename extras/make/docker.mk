...
docker.restart: clean
	@if [ "${stage}" == "" ]; then \
		...
		$(docker-compose) -f "${PATH_DOCKER_COMPOSE}"/"${stage}".yml --project-name "${PROJECT}-${stage}" restart; \
	fi
...
docker.build: clean
	...
	@if [ -z "${stage}" ] && [ -z "${service}" ]; then \
		...
		$(docker-compose) -f "${PATH_DOCKER_COMPOSE}"/dev.yml --project-name "${PROJECT}" build ${service} ${args}; \
	elif [ -n "${stage}" ] && [ -z "${service}" ]; then \
		$(docker-compose) -f "${PATH_DOCKER_COMPOSE}"/"${stage}".yml --project-name "${PROJECT}-${stage}" build ${args}; \
	fi
...
docker.down: clean
	...
	@if [ "${stage}" == "" ]; then \
		...
		$(docker-compose) -f "${PATH_DOCKER_COMPOSE}"/"${stage}".yml --project-name "${PROJECT}-${stage}" down --remove-orphans; \
	fi
...
docker.exec: clean
	...
	@if [ -z "${stage}" ]; then \
		...
		$(docker-compose) -f "${PATH_DOCKER_COMPOSE}"/"${stage}".yml --project-name "${PROJECT}-${stage}" exec ${service} ${command} ; \
	fi
...
docker.stop: clean
	...
	@if [ "${stage}" == "" ]; then \
		...
		$(docker-compose) -f "${PATH_DOCKER_COMPOSE}"/"${stage}".yml --project-name "${PROJECT}-${stage}" stop; \
	fi
...
docker.up: clean
	...
	@if [ "${stage}" == "" ]; then \
		...
		$(docker-compose) -f "${PATH_DOCKER_COMPOSE}"/"${stage}".yml --project-name "${PROJECT}-${stage}" up --remove-orphans; \
	fi
...
docker.list: clean
	...
	@if [ "${stage}" == "" ]; then \
		...
		$(docker-compose) -f "${PATH_DOCKER_COMPOSE}"/"${stage}".yml --project-name "${PROJECT}-${stage}" ps; \
	fi
